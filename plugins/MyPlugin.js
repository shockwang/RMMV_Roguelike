...
(function() {
    ...
    function playerCanSee(x, y) {
        if (Math.abs($gamePlayer._x - x) <= 1 && Math.abs($gamePlayer._y - y) <= 1) {
            return true;
        }
        return false;
    }
    ...
    MapUtils.translateMap = function(rawData){
        ...
        for (var i = 0; i < mapData.length; i++) {
            ...
            for (var j = 0; j < rawData[0].length; j++) {
                mapData[i][j] = new MapData(floorCenter);
            }
        }
    ...
    MapUtils.drawMap = function(mapData, mapArray) {
        ...
        for (var j = 0; j < mapData[0].length; j++) {
            for (var i = 0; i < mapData.length; i++) {
                if (playerCanSee(i, j) || mapData[i][j].isExplored) {
    ...
    DataManager.isMapLoaded = function() {
        console.log("$dataMap: " + $dataMap + ", $gameMap: " + $gameMap);
